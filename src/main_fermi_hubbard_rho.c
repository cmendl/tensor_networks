...
int main(int argc, char *argv[])
{
	...
	duprintf("Computing exp(-beta H/2) for the Fermi-Hubbard Hamiltonian on a chain with parameters:\n");
	...
	sprintf(filename, "%s/fermi_hubbard_L%i_beta%g_D.dat",       argv[2], L, params.beta);  WriteData(filename, D_beta, sizeof(size_t), L + 1, false);
