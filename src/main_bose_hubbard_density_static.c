...
int main(int argc, char *argv[])
{
	...
	duprintf("Computing static density-density correlations for the Bose-Hubbard model on a chain with parameters:\n");
	...
	duprintf("\nComputing static density-density correlations...\n");

	mpo_t XA, XB;
	CopyMPO(&rho_beta, &XA);
	CopyMPO(&rho_beta, &XB);

	double *nn_corr = (double *)MKL_malloc(L*L*sizeof(double), MEM_DATA_ALIGN);
	double *n_avr_A = (double *)MKL_malloc(L  *sizeof(double), MEM_DATA_ALIGN);
	double *n_avr_B = (double *)MKL_malloc(L  *sizeof(double), MEM_DATA_ALIGN);

	int jB;
	for (jB = 0; jB < L; jB++)
	{
		// backup copy of tensor at site 'jB'
		tensor_t XBj;
		CopyTensor(&XB.A[jB], &XBj);

		// apply number operator from the bottom at site 'jB'
		ApplySingleSiteBottomOperator(&XB.A[jB], &bn);

		n_avr_B[jB] = ComplexReal(MPOTraceProduct(&rho_beta, &XB));

		int jA;
		for (jA = 0; jA < L; jA++)
		{
			// backup copy of tensor at site 'jA'
			tensor_t XAj;
			CopyTensor(&XA.A[jA], &XAj);

			// apply number operator from the top at site 'jA'
			ApplySingleSiteTopOperator(&XA.A[jA], &bn);

			// <n_jB n_jA> density-density correlation
			nn_corr[jA + L*jB] = ComplexReal(MPOTraceProduct(&XA, &XB));

			if (jB == 0)
			{
				n_avr_A[jA] = ComplexReal(MPOTraceProduct(&XA, &rho_beta));
			}
	...
	sprintf(filename, "%s/bose_hubbard_L%i_M%zu_nn_corr.dat", argv[2], L, d - 1); WriteData(filename, nn_corr, sizeof(double), L*L, false);
	sprintf(filename, "%s/bose_hubbard_L%i_M%zu_n_avr_A.dat", argv[2], L, d - 1); WriteData(filename, n_avr_A, sizeof(double), L,   false);
	sprintf(filename, "%s/bose_hubbard_L%i_M%zu_n_avr_B.dat", argv[2], L, d - 1); WriteData(filename, n_avr_B, sizeof(double), L,   false);
